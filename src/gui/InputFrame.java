...
public class InputFrame extends JFrame implements MouseMotionListener {
	...
	private void incorrect() {
		DateFormat dateFormat = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
		Date date = new Date();
		logger.entry(dateFormat.format(date) + "," + 2 + "," + difficulty + ","
				+ step + "," + questiondata.getText().replace('\n', ',') + ",");
		...
		int action = RewardFunction.EndStep(understanding,
				need);
		switch (action) {
		case (0):
		...
		case (2):
			...
			cont.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					int skip = 0;
					double[] understanding = StudentData.student
							.find_probability(new Vertex<String>(
									"Understanding1", 3));
					if (question.skippable.get(step))
						skip = RewardFunction.skipStep(understanding);
					if (skip != 0)
						while (question.skippable.get(step + 1))
							step++;
					step++;
					filldata();
					cont.setVisible(false);
	...
	private void correct() {
		...
		cont.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int skip = 0;
				double[] understanding = StudentData.student
						.find_probability(new Vertex<String>("Understanding1",
								3));
				if (question.skippable.get(step))
					skip = RewardFunction.skipStep(understanding);
				if (skip != 0)
	...
	private void showdone() {
		...
		JOptionPane
				.showOptionDialog(
						this,
						"You have finished this exercise, time to move on to a new one.",
						"Exercise Complete", JOptionPane.DEFAULT_OPTION,
	...
	private void close() {
		...
		if (understanding == 0) {
			if (difficulty == 0)
				frame = new Instruction(0, 2);
			else
				frame = new InputFrame(difficulty - 1);
		} else {
			if (difficulty == 2)
				frame = new Instruction(2, 0);
			else
				frame = new InputFrame(difficulty + 1);
		}
		WindowEvent wev = new WindowEvent(this, WindowEvent.WINDOW_CLOSING);
